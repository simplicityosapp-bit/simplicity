...
// screens it auto-shows a back chevron above the card. Pass back={false} to force
// it off.
//
...
export default function ScreenHead({ title, meta = [], tagline, onAdd, addLabel, back }) {
  ...
  return (
    <View style={styles.wrap}>
      {showBack ? (
        <Pressable style={[styles.back, { alignSelf: flip ? 'flex-end' : 'flex-start' }]} onPress={() => nav.goBack()} hitSlop={10} accessibilityRole="button">
          <BackIcon size={26} strokeWidth={1.8} color={colors.brand} />
        </Pressable>
      ) : null}
...
const styles = StyleSheet.create({
  ...
  // Card + FAB row. flex-start so the FAB pins to the top of the row regardless of
  // how tall the card is (some screens carry meta chips, others don't) — the add
  // button then sits at the SAME position on every screen.
  top: { flexDirection: 'row', alignItems: 'flex-start', gap: 12, paddingTop: 4 },
  topFlip: { flexDirection: 'row-reverse' },
  // Glass card, ~62% like the web .screen-head; title on top, meta + tagline below.
  // alignItems stretch so the title/tagline fill the width and their textAlign
  // (start-edge) takes effect in both LTR and RTL.
