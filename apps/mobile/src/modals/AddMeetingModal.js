...
export default function AddMeetingModal({ open, onClose, onSave, clients = [] }) {
      ...
      <View style={styles.field}>
        ...
        {clients.length ? (
          <View style={styles.chips}>
            {clients.map((c) => {
              const on = form.client_id === c.id
              return (
                <Pressable key={c.id} style={[styles.chip, on && styles.chipOn]} onPress={() => { set('client_id', c.id); if (err) setErr('') }}>
          ...
          <Text style={styles.hint}>{i18n.t('modalsTask:meeting.pickClient')}</Text>
        )}
      </View>
...
const styles = StyleSheet.create({
  ...
  chip: { paddingVertical: 8, paddingHorizontal: 14, borderRadius: 999, borderWidth: 1, borderColor: colors.border, backgroundColor: colors.cardFlat, maxWidth: '100%' },
  chipOn: { backgroundColor: colors.brand, borderColor: colors.brand },
  chipText: { fontSize: 14, color: colors.text },
  chipTextOn: { color: colors.onBrand, fontWeight: '600' },
