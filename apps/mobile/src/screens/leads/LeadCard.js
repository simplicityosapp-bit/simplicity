...
export default function LeadCard({ lead, onEdit, onConvert, onDelete, onMove, sources = [], statuses = [], dragHandlers = null, dragging = false }) {
  ...
  return (
    <Pressable style={[styles.card, dragging && styles.dragging]} onPress={() => onEdit?.(lead)} {...(dragHandlers || {})}>
      {onDelete ? (
      ...
      {lead.follow_up_date ? (
        <View style={styles.line}>
          <Clock size={11} strokeWidth={1.6} color={overdue ? colors.amberWarn : colors.textFaint} />
...
const styles = StyleSheet.create({
  ...
  sub: { flexDirection: 'row', alignItems: 'center', gap: 5 },
  ...
  overdue: { color: colors.amberWarn, fontWeight: '600' },
  ...
  convertText: { fontSize: 11, fontWeight: '500', color: colors.positive },
  iconBtn: { width: 28, height: 28, borderRadius: 8, alignItems: 'center', justifyContent: 'center', backgroundColor: colors.cardFlat },
})
