...
export function usePointerDnd({ onDrop } = {}) {
  ...
  const activate = useCallback((id, x, y, sourceEl) => {
    ...
    /* A popover that happened to be open on the source would otherwise be
       frozen into the floating ghost and travel with it. The ghost is a
       picture of the card, not an interactive copy, so any open menu is
       stripped and its trigger reset. Keyed on the ARIA role rather than a
       class, so this holds for whatever popover a card grows next. */
