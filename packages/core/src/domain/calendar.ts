...
/** Anything our date helpers accept — a Date, an ISO string, or an epoch ms. */
export type DateInput = Date | string | number

/** The Hebrew-calendar breakdown of a civil date (see hebrewParts). */
...
/** A Hebrew month as { civil 1st-of-month date, Hebrew month name }. */
...
export const weekdayNamesShort = (lng?: string): string[] =>
  i18n.t('calendar:greg.weekdaysShort', { returnObjects: true, lng }) as unknown as string[]
export const weekdayNamesLong = (lng?: string): string[] =>
  i18n.t('calendar:greg.weekdaysLong', { returnObjects: true, lng }) as unknown as string[]
export const monthNamesLong = (lng?: string): string[] =>
  i18n.t('calendar:greg.monthsLong', { returnObjects: true, lng }) as unknown as string[]
export const monthNamesShort = (lng?: string): string[] =>
  i18n.t('calendar:greg.monthsShort', { returnObjects: true, lng }) as unknown as string[]

export function startOfDay(d: DateInput): Date {
  const x = new Date(d)
  x.setHours(0, 0, 0, 0)
  return x
}

export function endOfDay(d: DateInput): Date {
  const x = new Date(d)
  x.setHours(23, 59, 59, 999)
  return x
}

export function isSameDay(a?: DateInput | null, b?: DateInput | null): boolean {
  if (!a || !b) return false
...
export function weekStartIndex(weekStartPref?: string): number {
  return weekStartPref === 'monday' ? 1 : 0
}

/* Walk backward from `d` until we land on the week-start day. */
export function startOfWeek(d: DateInput, weekStart: string = 'sunday'): Date {
...
export function monthGrid(monthDate: Date, weekStart: string = 'sunday'): Date[] {
...
export function fmtDayLabel(date: DateInput): string {
...
function hebLettersUnder1000(n: number): string {
...
export function hebrewParts(date: DateInput): HebrewParts {
  const x = new Date(date)
  x.setHours(12, 0, 0, 0)
  const p = _hebFmt.formatToParts(x).reduce((a, part) => { a[part.type] = part.value; return a }, {} as Record<string, string>)
...
export function hebrewDayNum(date: DateInput): string {
...
export function isSameHebrewMonth(a: DateInput, b: DateInput): boolean {
...
export function startOfHebrewMonth(date: DateInput): Date {
...
export function stepHebrewMonth(date: DateInput, dir: number): Date {
...
function hebrewMonthBase(name: string): string {
...
export function stepHebrewYear(date: DateInput, dir: number): Date {
...
export function hebrewMonthGrid(date: DateInput, weekStart: string = 'sunday'): Date[] {
...
export function hebrewMonthsOfYear(date: DateInput): HebrewMonth[] {
...
export function hebrewMonthLabel(date: DateInput): string {
  const p = hebrewParts(date)
  return `${p.month} ${p.yearText}`
}

/* "יום ראשון · ט׳ בתמוז תשפ״ו" — full day label for the day view. */
export function fmtHebrewDayLabel(date: DateInput): string {
  const d = new Date(date)
  const p = hebrewParts(date)
