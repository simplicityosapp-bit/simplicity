/* ════════════════════════════════════════════════════════════════
   LEGAL CONSENT — versions + helpers, for both apps.
   ════════════════════════════════════════════════════════════════
   ...
   the signup moment (before the email is confirmed / a session exists). The
   durable append-only record is public.user_consent, fed from the same
   blocks by consentRowsFromMetadata.
   ...
   Moved here from apps/web/src/lib/legal.js. The phone used to keep its own
   copy of the three versions under a "keep in sync" comment and had no
   re-acceptance gate at all, so a phone-only user never saw a policy change.
   One source now: a bump here re-prompts in both apps.
   ════════════════════════════════════════════════════════════════ */
...
export type ConsentKind = 'privacy' | 'dpa' | 'terms' | 'marketing'
...
export function buildConsent(
  { marketing = false }: { marketing?: boolean } = {},
...
export function buildReacceptance(now: string = new Date().toISOString()) {
  return {
    privacy_accepted_at: now,
    privacy_version: PRIVACY_VERSION,
    dpa_accepted_at: now,
...
export function needsReacceptance(user?: { user_metadata?: ConsentMetadata | null } | null): boolean {
...
export function marketingConsent(user?: { user_metadata?: ConsentMetadata | null } | null): boolean {
  return !!user?.user_metadata?.marketing_consent
}

/* Derive the durable user_consent rows from a consent block (user_metadata OR
   a pending consent — same shape). Records the privacy + DPA + terms
   acceptances and the marketing choice (opt-in OR opt-out) at the moment it
   was made. `accepted_at` is the dedup key, so re-deriving + re-recording is
   idempotent. */
export function consentRowsFromMetadata(md: ConsentMetadata | null | undefined, source = 'backfill'): ConsentRow[] {
  if (!md) return []
