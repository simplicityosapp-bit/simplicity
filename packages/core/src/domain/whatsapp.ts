/* ════════════════════════════════════════════════════════════════
   ...
   Moved here from apps/web/src/lib so the phone builds the same links.
   Its five copies only stripped non-digits, so an Israeli number as
   coaches type it — 050-1234567 — became wa.me/0501234567, a number
   WhatsApp cannot resolve, and every WhatsApp button in the app opened
   onto an error.
   ════════════════════════════════════════════════════════════════ */
...
export function waLink(phone: unknown, text?: string | null): string {
...
export function fillTemplate(template: string | null | undefined, vars: Record<string, unknown> = {}): string {
  if (!template) return ''
  return String(template).replace(
    /\{\{\s*(\w+)\s*\}\}/g,
    (_, key: string) => (vars[key] == null ? '' : String(vars[key])),
  )
}
...
export function resolveMessage(customTemplate: string | null | undefined, vars: Record<string, unknown>, fallback: string): string {
