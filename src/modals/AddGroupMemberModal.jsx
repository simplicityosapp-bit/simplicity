  ...
  return (
    <Modal open={open} onClose={close} title={t('addGroupMember.title')}>
      ...
      <div className="m-field">
        <label className="m-label">{t('addGroupMember.client')}</label>
        {availableClients.length ? (
          ...
          <p className="m-error">{t('addGroupMember.allMembers')}</p>
        )}
      </div>
      <div className="m-field">
        <label className="m-label">{t('addGroupMember.joinDateOptional')}</label>
      ...
      <div className="m-actions">
        ...
        <button type="button" className="m-btn-save" onClick={submit} disabled={busy || !availableClients.length}>{busy ? t('common.saving') : t('addGroupMember.addAction')}</button>
      </div>
    </Modal>
  )
}
