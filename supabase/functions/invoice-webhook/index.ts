...
Deno.serve(async (req) => {
  try {
    ...
    // Income import is opt-in: `auto_import` ON = stage incoming docs for the
    // user's approval; OFF (default) = ignore them entirely (no import).
    if (!integ.auto_import) return ok()
    ...
    // Upsert + ignoreDuplicates makes a racy SUMIT re-delivery a no-op.
